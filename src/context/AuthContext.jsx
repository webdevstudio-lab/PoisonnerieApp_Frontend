...
export const AuthProvider = ({ children }) => {
  ...
  const logout = useCallback(async () => {
    try {
      ...
      localStorage.removeItem("_appPoissonerie_user");
      setUser(null);
  ...
  const checkAuthStatus = useCallback(() => {
    const userStr = localStorage.getItem("_appPoissonerie_user");
  ...
  const login = (userData) => {
    ...
    localStorage.setItem("_appPoissonerie_user", JSON.stringify(normalized));
    setUser(normalized);
    setIsAuthenticated(true);
    setLoading(false);
  };
